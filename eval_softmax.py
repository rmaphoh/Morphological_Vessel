...
def eval_net(epoch, net, loader, device, mask, mode):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for batch in loader:
            ...
            with torch.no_grad():
                ...
                mask_pred = decode_one_hot(mask_pred)
            ...
            if mask:
                ...
                if mode== 'vessel':
                    ...
                    mask_pred_sigmoid_cpu = mask_pred.detach().cpu().numpy()
                    mask_pred_sigmoid_cpu = np.squeeze(mask_pred_sigmoid_cpu, axis=1)
                    true_masks_cpu = true_masks.detach().cpu().numpy()
                    ...
                    encoded_pred = np.zeros(mask_pred_sigmoid_cpu.shape, dtype=int)
                    
                    
                    #print(np.shape(encoded_pred))
                    #print(np.shape(true_masks_cpu))
                    #print(np.unique(true_masks_cpu))
                    encoded_gt = np.zeros(mask_pred_sigmoid_cpu.shape[0:2], dtype=int)
                    encoded_mask = np.zeros((mask_pred_sigmoid_cpu.shape[0:2]), dtype=int)
                    
                    
                    # convert white pixels to green pixels (which are ignored)
                    white_id = np.where(np.logical_and(true_masks_cpu[:,:,0] == 1, true_masks_cpu[:,:,1] == 1, true_masks_cpu[:,:,2] == 1))
                    black_id = np.where(np.logical_and(true_masks_cpu[:,:,0] == 0, true_masks_cpu[:,:,1] == 0, true_masks_cpu[:,:,2] == 0))
                    artery_id = np.where(np.logical_and(true_masks_cpu[:,:,0] == 1, true_masks_cpu[:,:,1] == 0, true_masks_cpu[:,:,2] == 0))
                    vein_id = np.where(np.logical_and(true_masks_cpu[:,:,0] == 0, true_masks_cpu[:,:,1] == 0, true_masks_cpu[:,:,2] == 1))
                    uncertain_id = np.where(np.logical_and(true_masks_cpu[:,:,0] == 0, true_masks_cpu[:,:,1] == 1, true_masks_cpu[:,:,2] == 0))
 
                    #print('white, ',np.shape(white_ind))

                    #print(type(white_ind))

                    if white_id[0].size != 0:
                    
                        encoded_mask[black_id] = 0
                        encoded_mask[artery_id] = 1
                        encoded_mask[vein_id] = 2
                        encoded_mask[uncertain_id] = 3
                        encoded_mask[white_id] = 4
                        
                    
                        
                    # translate the images to arrays suited for sklearn metrics
                    arteriole = np.where(encoded_mask==1); encoded_gt[arteriole] = 1
                    venule = np.where(encoded_mask==2); encoded_gt[venule] = 2
                    #uncertainty = np.where(np.logical_and(true_masks_cpu[:,:,1] == 1, true_masks_cpu[:,:, 0] == 0, true_masks_cpu[:,:, 2] == 0)); encoded_gt[uncertainty] = 3

                    arteriole = np.where(mask_pred_sigmoid_cpu==1); encoded_pred[arteriole] = 1
                    venule = np.where(mask_pred_sigmoid_cpu==2); encoded_pred[venule] = 2
                    ...
                    vessel_point = np.where(np.logical_and(encoded_gt[:,:]>0, encoded_pred[:,:,0] > 0))
                    
                    encoded_pred = encoded_pred.squeeze()
